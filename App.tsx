...
const App: React.FC = () => {
  ...
  const renderContent = () => {
    // Show Birthday view immediately if selected, no need to wait for sheet data
    if (activeTab === TabView.BIRTHDAYS) {
       return <BirthdayView />;
    }
  ...
  return (
    <div className="min-h-screen bg-elfsborg-black text-gray-100 font-sans selection:bg-elfsborg-yellow selection:text-black flex flex-col">
      ...
      <main className="max-w-6xl mx-auto px-4 flex-grow w-full">
        ...
        <div className="flex border-b border-elfsborg-border mb-6 mt-6 md:mb-8 md:mt-8 overflow-x-auto no-scrollbar">
          <button
            onClick={() => setActiveTab(TabView.TRANSFERS)}
            className={`flex-1 md:flex-none pb-3 px-4 md:px-6 text-sm font-bold uppercase tracking-wider transition-all duration-200 border-b-2 text-center whitespace-nowrap ${
          ...
          <button
            onClick={() => setActiveTab(TabView.BIRTHDAYS)}
            className={`flex-1 md:flex-none pb-3 px-4 md:px-6 text-sm font-bold uppercase tracking-wider transition-all duration-200 border-b-2 text-center whitespace-nowrap ${
              activeTab === TabView.BIRTHDAYS
                ? 'border-elfsborg-yellow text-white'
                : 'border-transparent text-gray-500 hover:text-gray-300'
            }`}
          >
        ...
        {!API_URL && !loading && activeTab !== TabView.BIRTHDAYS && (
